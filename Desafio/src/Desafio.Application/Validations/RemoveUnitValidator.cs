...
public class RemoveUnitValidator : AbstractValidator<Unit>
{
    ...
    public RemoveUnitValidator(IUnitService unitService)
    {
        ...
        RuleFor(x => x.Acronym)
                ...
                .MustAsync(HasNotBeenUsedBeforeAsync).WithMessage("It's not possible to remove a unit that is being used in a product.")
                .Length(2, 4)
    ...
    private async Task<bool> HasNotBeenUsedBeforeAsync(string acronym, CancellationToken token)
    {
        //Verifica se a unidade já foi utilizada em um produto
        return await _unitService.HasNotBeenUsedBeforeAsync(acronym);
    }
}
